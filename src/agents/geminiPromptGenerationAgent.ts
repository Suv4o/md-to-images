...
IMPORTANT: You must respond with ONLY valid JSON. No markdown, no code fences, no extra text. Just the JSON object.`;

export async function generateGeminiPrompts(options: GeneratePromptsOptions): Promise<ImagePrompt[]> {
    const { articleContent, customInstructions, hasReferenceImage } = options;

    logInfo(`Starting prompt generation with Gemini (${CONFIG.GEMINI_TEXT_MODEL})...`);
...
5. Draw inspiration from the article content${customInstructions ? "\n6. Follow the custom instructions provided above" : ""}${hasReferenceImage ? "\n" + (customInstructions ? "7" : "6") + ". Reference the style from the provided reference image in each prompt" : ""}`;
    ...
    if (!text) {
        throw new Error("Failed to generate prompts: No text received from Gemini");
    }

    let parsed: { prompts: ImagePrompt[] };
    try {
